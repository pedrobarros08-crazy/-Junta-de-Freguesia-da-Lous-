            ...
            <td>
                ...
                if (!$serverName) {
                    ...
                    if ($testConn !== false) {
                        ...
                        error_log('verificar_sqlsrv.php: falha de ligação de teste - ' . print_r($errors, true));
                        $msg = is_production()
                            ? 'Erro interno ao ligar à base de dados.'
                            : (isset($errors[0]['message']) ? $errors[0]['message'] : 'Erro desconhecido');
                        echo '<span class="erro">❌ ' . htmlspecialchars($msg, ENT_QUOTES, 'UTF-8') . '</span>';
                    }
                }
                ?>
            </td>
